...
export default async function MethodologyPage() {
  ...
  const positions: PositionCardData[] = [
    ...
    },
    {
      ...
      availabilityNote: "Data available from 2016. Aggregated directly from play-by-play (nflverse player_stats doesn't carry detailed punting columns). Qualified threshold is 40 punts. Two-component formula: net average (primary, captures distance + return prevention) and inside-20 placement rate (orthogonal placement skill — best Pro Bowl validity in the audit). v1.1 removed `blocked_rate` (was -0.05 in v1) — most blocks are snap/protection failures rather than punter skill, and the event is too rare to carry weight. The over-expected approach (EPA per punt) was tested as an alternative single-component formula but did NOT dominate net average on YoY or validity — the K v1.1 lesson didn't fully generalize because punt EPA mixes punter skill with opponent quality and field position. The leaderboard shows FORMULA columns (Net avg, Inside 20%) plus CONTEXT columns (Gross avg, Block%, Long, TB%) for reader recognition. Punter Pro Bowl voting is the noisiest of any graded position (only 2 picks per year out of ~30 qualified) — expect the lowest validity baseline.",
      top: pTop,
    },
  ];
