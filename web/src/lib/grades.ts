...
const COMPONENT_FORMATS: Record<string, ComponentFormat> = {
  ...
  // --- OL v1 (ADR-0025) — TEAM-LEVEL grading ---
  ol_yards_before_contact_per_carry: {
    label: "YBC / carry",
    suffix: " yd",
    formatValue: (v) => v.toFixed(2),
    description:
      "Yards Before Contact per carry — the distance the team's RBs travel before being touched, averaged across all carries. Isolates OL run-blocking from RB after-contact ability (after-contact yards belong to the RB; before-contact yards belong to the OL). Primary run-block signal — best YoY in the audit (+0.42).",
    sampleNoun: "carry",
  },
  ol_pressure_proxy_per_dropback: {
    label: "Pressure %",
    suffix: "%",
    formatValue: pctFraction(1),
    description:
      "(Sacks + QB hits) per dropback. The cleanest pass-block signal available without paid PFF data. Subsumes both standalone sack rate and standalone QB-hit rate (audit max_r ≈ 0.86–0.96 with each). Lower is better — measures how often the OL lets the QB take meaningful contact.",
    sampleNoun: "dropback",
  },
};
...
const COMPONENT_WEIGHTS: Record<string, number> = {
  ...
  ol_yards_before_contact_per_carry:   0.45,
  ol_pressure_proxy_per_dropback:     -0.45,
};
