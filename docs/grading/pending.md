...
| 15 | New position | Punters v1 (full audit-first process) | 0.5-1 day | **SHIPPED 2026-05-14** ([audit](audits/2026-05-14-exhaustive-p.md), [ADR](../adr/0024-p-v1-grading-formula.md)) — Option B multi-component formula: net_avg (+0.55), inside_20_rate (+0.30), blocked_rate (-0.05). Critical lesson: K v1.1's FGOE-dominance pattern did NOT generalize — `p_epa_per_punt` lost to `p_net_avg` on YoY (+0.269 vs +0.355) and validity (+0.163 vs +0.166). Validity +0.122 (lowest of any position). Face-check: Jack Fox #1 (1st-Team All-Pro), both 2024 Pro Bowl Ps in top 3. |
