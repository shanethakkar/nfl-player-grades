"""iDL (interior defensive lineman) v1 grading pipeline (ADR-0021).

Flow:
    1. ``extract_features``: reads ``pfr_def_pass_rush`` (same table as EDGE,
       ingested by ``ingest/pfr_dl.py``) + ``player_seasons`` (for
       snaps_defense), filters to iDL players, computes per-snap rate metrics.
...
from nfl_grades.grading.weights import (
    ...
    IDL_V1_QUALIFIED_MIN_SNAPS,
    IDL_V1_RAW_VALUE_COLS,
    IDL_V1_SAMPLE_SIZE_COLS,
    IDL_V1_SHRINKAGE_K,
    IDL_V1_WEIGHTS,
)
...
def run(season: int) -> RunResult:
    if season < PFR_DEF_PASS_RUSH_MIN_SEASON:
        logger.warning(
            "iDL grading requires season >= %d; got %d — skipping",
            PFR_DEF_PASS_RUSH_MIN_SEASON, season,
        )
        return RunResult(season, 0, 0, 0, 0)

    engine = get_engine()
    with pipeline_run("grading:idl", season=season) as handle:
        with engine.begin() as conn:
            features = extract_features(conn, season)
            if features.empty:
                logger.warning("no iDL players found for season %d", season)
        ...
        result = RunResult(
            season=season,
            n_idls_total=len(graded),
            n_idls_qualified=int(graded["qualified"].sum()),
            stat_components_written=n_components,
            season_grades_written=n_grades,
        )
        handle.rows_written = n_grades
        handle.note(
            f"idls_total={result.n_idls_total} "
            f"idls_qualified={result.n_idls_qualified}"
        )
    return result
...
def extract_features(conn: Connection, season: int) -> pd.DataFrame:
    rows = (
        conn.execute(
            _FEATURES_SQL,
            {"season": season, "min_snaps": IDL_V1_MIN_SNAPS_TO_GRADE},
        )
        .mappings()
        .all()
    )
...
def compute_grades(features: pd.DataFrame, season: int) -> pd.DataFrame:
    df = features.copy()
    df["qualified"] = df["snaps_defense"] >= IDL_V1_QUALIFIED_MIN_SNAPS
    df["confidence"] = (
        df["snaps_defense"].astype(float) / IDL_V1_CONFIDENCE_FULL_SNAPS
    ).clip(upper=1.0)
    ...
    for component, raw_col in IDL_V1_RAW_VALUE_COLS.items():
        n_col = IDL_V1_SAMPLE_SIZE_COLS[component]
        raw = df[raw_col]
        n = df[n_col]
        k = IDL_V1_SHRINKAGE_K[component]
    ...
    df["composite_z"] = composite.combine(z_frame, IDL_V1_WEIGHTS)
...
def write_results(conn: Connection, graded: pd.DataFrame, season: int) -> tuple[int, int]:
    components = list(IDL_V1_RAW_VALUE_COLS.keys())
