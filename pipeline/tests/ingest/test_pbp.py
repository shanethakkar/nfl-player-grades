...
class TestReplaceSeason:
    ...
    def test_empty_clears_season(self, conn) -> None:
        ...
        count = conn.execute(text("SELECT COUNT(*) FROM plays WHERE season=1999")).scalar()
        assert count == 0

    def test_nulls_round_trip(self, conn) -> None:
        # Most columns NULL — exercises every nullable column.
        rows = [self._row(play_id=42, epa=None, qb_dropback=None, play_desc=None)]
        assert _replace_season(conn, rows, 1999) == 1
        row = conn.execute(
            text("SELECT epa, qb_dropback, play_desc FROM plays WHERE season=1999 AND play_id=42")
        ).first()
    ...
    def test_does_not_affect_other_seasons(self, conn) -> None:
        ...
        # 1998 must survive.
        count_1998 = conn.execute(text("SELECT COUNT(*) FROM plays WHERE season=1998")).scalar()
        assert count_1998 == 1
